...
def load_and_test_svm():
    ...
    # bayes = cv2.ml.NormalBayesClassifier_load('./generated-files/bayes_shapes_model.yml')
    tree = cv2.ml.DTrees_load('./generated-files/tree_shapes_model.yml')
    files = glob.glob('./shapes/testing/*')
    for f in files:
        hu_moments = hu_moments_of_file(f)
        sample = np.array([hu_moments], dtype=np.float32)
        testResponse = tree.predict(sample)[1]
